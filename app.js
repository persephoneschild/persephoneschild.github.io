...
function showRoute() { const route = location.hash.replace('#', '') || 'home'; const validRoute = ['home', 'audios', 'hadestown', 'videos', 'wants', 'cart'].includes(route) ? route : 'home'; document.querySelectorAll('.view').forEach((view) => view.classList.toggle('active', view.dataset.view === validRoute || (view.dataset.view === 'collection' && COLLECTION_ROUTES[validRoute]))); document.querySelectorAll('[data-route]').forEach((link) => link.classList.toggle('active', link.dataset.route === validRoute || (link.dataset.route === 'collection' && COLLECTION_ROUTES[validRoute]))); if (COLLECTION_ROUTES[validRoute] && state.recordings.length) renderRecordings(); window.scrollTo({ top: 0, behavior: 'smooth' }); }
